...
const RichEditor = dynamic(
  ...
  { ssr: false, loading: () => <div className="grid h-64 place-items-center rounded-xl border border-border/60 bg-surface-2/60 text-sm text-muted-foreground">Loading editor…</div> },
);
    ...
    <div className="mx-auto max-w-5xl">
      <div className="mb-6 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Link href="/admin/blogs" className="grid size-9 place-items-center rounded-lg border border-border/60">
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        ...
        <div className="space-y-5">
          <Field label="Title">
            <TextInput value={form.title} onChange={(e) => onTitle(e.target.value)} placeholder="An amazing headline" />
          </Field>
        ...
        <div className="space-y-5">
          <div className="space-y-4 rounded-2xl border border-border/60 bg-card/50 p-5">
            <Field label="Status">
              <SelectInput
                value={form.status}
                onChange={(e) => set("status", (e.target as HTMLSelectElement).value as Blog["status"])}
                options={[
          ...
          <div className="space-y-4 rounded-2xl border border-border/60 bg-card/50 p-5">
            ...
            <Field label="Role">
              <TextInput value={form.authorRole ?? ""} onChange={(e) => set("authorRole", e.target.value)} />
            </Field>
            <Field label="Avatar">
              <ImageUploader value={form.authorAvatar} onChange={(url) => set("authorAvatar", url)} label="Avatar" />
            </Field>
          </div>
          ...
          <div className="space-y-4 rounded-2xl border border-border/60 bg-card/50 p-5">
            ...
            <Field label="OG image">
              <ImageUploader value={form.ogImage} onChange={(url) => set("ogImage", url)} label="OG image" />
            </Field>
          </div>
        </div>
      </div>
